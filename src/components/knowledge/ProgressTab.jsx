  ...
  return (
    <div className="hyp-form">
      <div className="hyp-form-section">
        <label className="hyp-form-label">标题（选填）</label>
        <input
          className="hyp-name-input"
          placeholder="给这条假设起个短名，方便回溯，如「AI简历匹配」"
          value={name}
...
function HypothesisCard({ hypothesis }) {
  ...
  const updateHypothesis  = useAppStore((s) => s.updateHypothesis)
  ...
  const [editName,      setEditName]      = useState(hypothesis.name  || '')
  const [editTitle,     setEditTitle]     = useState(hypothesis.title)
  const [editMvp,       setEditMvp]       = useState(hypothesis.mvp)
  ...
  const handleSaveEdit = () => {
    if (!editTitle.trim() || !editMvp.trim()) return
  ...
  const handleCancelEdit = () => {
    ...
    setEditMvp(hypothesis.mvp)
    setEditing(false)
  }

  const displayName = hypothesis.name || '未命名假设'
  ...
  return (
    <div className="hyp-card">
      ...
      <div className="hyp-card-header">
        ...
        {/* 标题区（点击同样可折叠） */}
        <div className="hyp-card-title-area" onClick={() => setCollapsed((v) => !v)}>
          <span className="hyp-card-name">{displayName}</span>
          <div className="hyp-card-timestamps">
            <span className="hyp-card-date">创建 {formatDate(hypothesis.createdAt)}</span>
            {hypothesis.updatedAt && (
              <span className="hyp-card-date">编辑 {formatDate(hypothesis.updatedAt)}</span>
            )}
          </div>
        </div>
        ...
        <div className="hyp-card-actions">
          ...
          {confirmDelete ? (
            ...
            !editing && (
              <button className="hyp-icon-btn del" title="删除" onClick={() => setConfirmDelete(true)}>×</button>
            )
          )}
        </div>
      </div>
      ...
      {!collapsed && (
        <>
          {editing ? (
            /* 编辑模式 */
            <div className="hyp-edit-form">
              <div className="hyp-form-section">
                <label className="hyp-form-label">标题（选填）</label>
                <input
                  className="hyp-name-input"
                  placeholder="给这条假设起个短名"
                  value={editName}
              ...
              <div className="hyp-form-section">
                <label className="hyp-form-label">① 可验证假设</label>
                <textarea
                  className="hyp-textarea"
                  rows={5}
                  value={editTitle}
              ...
              <div className="hyp-form-section">
                <label className="hyp-form-label">② MVP 设计</label>
                <textarea
                  className="hyp-textarea"
                  rows={7}
                  value={editMvp}
              ...
              <div className="hyp-form-actions">
                <button className="motto-cancel-btn" onClick={handleCancelEdit}>取消</button>
                <button
                  className="motto-save-btn"
                  onClick={handleSaveEdit}
                  disabled={!editTitle.trim() || !editMvp.trim()}
                >
                  保存修改
                </button>
              </div>
